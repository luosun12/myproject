...
class NewListTest(TestCase):
    ...
    def test_redirects_after_POST(self):
        response = self.client.post('/lists/new',data={'item_text':'A new list item'})
        new_list = List.objects.first() #(1)
        self.assertRedirects(response,f'/lists/{new_list.id}/') #(2)
...
class NewItemTest(TestCase):
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        self.assertEqual(Item.objects.count(),1) #(1)
        new_item = Item.objects.first() #(2)
        self.assertEqual(new_item.text,'A new item for an existing list') #(3)
        self.assertEqual(new_item.list,correct_list) #(4)
    ...
    def test_redirects_to_list_view(self):
        ...
        self.assertRedirects(response,f'/lists/{correct_list.id}/') #(2)
